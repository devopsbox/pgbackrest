    ...
    if ($strTest eq 'all' || $strTest eq 'full')
    {
        ...
        for (my $bCompress = false; $bCompress <= true; $bCompress++)
        {
            ...
            if ($bRemote)
            {
                ...
                # docker exec -u backrest test-0-backup /backrest/bin/pgbackrest --config=/home/vagrant/test/test-0/backup/pgbackrest.conf --stop-auto --type=incr --no-resume --stanza=db --backup-standby --db-standby-host=db-standby --db-standby-path=/home/vagrant/test/test-0/db-standby/db/base --db-standby-socket-path=/home/vagrant/test/test-0/db-standby/db --db-standby-user=vagrant --test --log-level-console=debug backup
